...
public class ModularSqrt {
     ...
     */
    public static int modularSqrt(BigInteger n, int p) {
        int pMod8 = p & 7;
        switch (pMod8) {
            case 1:
...
//                LOG.warn("Tonelli_Shanks() has been called with even p=" + p + " -> brute force search required.");
     ...
     * @return the modular sqrt t
     */
    private static int Tonelli_Shanks(BigInteger n, int p) {
        ...
        int z;
        for (z = 2; ; z++) {
            if (JacobiSymbol.jacobiSymbol(z, p) == -1) break;
        }
        // now z is found -> set c == z^Q
        int c = ModularPower.modPow(z, Q, p);
        int R = ModularPower.modPow(n, (Q + 1) >> 1, p);
        int t = ModularPower.modPow(n, Q, p);
        int M = S;
        while (t != 1) { // if t=1 then R is the result
            ...
            int i;
            for (i = 1; i < M; i++) {
                if (ModularPower.modPow(t, 1 << i, p) == 1) { // t^(2^i) == 1 (mod p) ?
                    foundI = true;
                    break;
                }
            }
            ...
            int b = ModularPower.modPow(c, 1 << (M - i - 1), p); // c^(2^(M-i-1))
            R = (int) ((R * (long) b) % p);
            c = (int) ((b * (long) b) % p);
            t = (int) ((t * (long) c) % p);
            M = i;
     ...
     */
    private static int Lagrange(BigInteger n, int p) {
        int t = ModularPower.modPow(n, (p + 1) >> 2, p);
        // return the smaller sqrt
        return t <= (p >> 1) ? t : p - t;
    }
     ...
     * @param n a positive integer having Jacobi(n|p) = 1
     */
    private static int case5Mod8(BigInteger n, int p) {
        int k = p >> 3; // for p == 5 (mod 8) we need k = (p-5)/8 = p>>3
        BigInteger n2 = n.shiftLeft(1);
        int g = ModularPower.modPow(n2, k, p);
     ...
     * @param n a positive integer having Jacobi(n|p) = 1
     */
    private static int bruteForce(BigInteger n, int p) {
     ...
     */
    public static BigInteger modularSqrtModPower(BigInteger n, BigInteger power, BigInteger last_power, BigInteger t) {
        // Barthel's e = (power - 2*last_power + 1)/2
        BigInteger f = power.subtract(last_power.shiftLeft(1)).add(BigInteger.ONE).shiftRight(1);
        // square root == n (mod p^exponent)
        // return the smaller sqrt
        return t.modPow(last_power, power).multiply(n.modPow(f, power)).mod(power);
    }
     ...
     * @return sqrt of n (mod p^exponent)
     */
    public static int modularSqrtModPower(BigInteger n, int power, int last_power, int t) {
        // Barthel's e = (power - 2*last_power + 1)/2
        int f = (power - (last_power << 1) + 1) >> 1;
        // square root == n (mod p^exponent)
        // return the smaller sqrt
        return (int) (ModularPower.modPow(t, last_power, power) * (long) ModularPower.modPow(n, f, power) % power);
    }
}
